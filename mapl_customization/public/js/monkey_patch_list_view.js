...
frappe.views.ListRenderer = Class.extend({
	...
	set_override_defaults: function() {
		if (this.settings.override_all_columns) {
		        this.settings.hide_activity = 1;
			this.settings.selectable = 0;
		        this.settings.hide_list_view_items =1;

			//hide_name_column Should be Explicitly Specified
		        //this.settings.hide_name_column = 1;
		}
	},
	...
	set_fields: function () {
		...
		//Monkey Here
		if ((typeof this.settings.hide_list_view_items === 'undefined') || !this.settings.hide_list_view_items) {
		...
		// additional fields
		//Monkey Here
		if (this.settings.add_fields && !this.settings.override_all_columns) {
			this.settings.add_fields.forEach(add_field);
		} else if (this.settings.override_all_columns) {
			this.settings.override_all_columns.forEach(function(d) {
	...
	set_columns: function () {
		...
		//Monkey Here
		//Total Colspan now a Variable and Increased to 24 from 12
		this.total_columns = 24;
		...
		//Monkey Here
		//if (this.settings.override_all_columns) { this.name_column.colspan = 0; }

		if (this.meta.title_field) {
			this.name_column.title = frappe.meta.get_docfield(this.doctype, this.meta.title_field).label;
		}
		this.columns.push(this.name_column);
		...
		// overridden
		var overridden = !this.settings.override_all_columns ? (this.settings.add_columns || []).map(function (d) {
			return d.content;
		}) : [];

		// custom fields in list_view
		var docfields_in_list_view = ((typeof this.settings.hide_list_view_items==='undefined') || !this.settings.hide_list_view_items) ?
		...
		docfields_in_list_view.forEach(function (d) {
			...
			if (me.total_colspans < me.total_columns) {
				me.add_column(d);
			}
		});
		...
		// additional columns
		//Monkey Here
		if (this.settings.add_columns && !this.settings.override_all_columns) {
			...
			this.settings.override_all_columns.forEach(function (d) {
				if (me.total_colspans < me.total_columns) {
					if (typeof d === 'string') {
						me.add_column(frappe.meta.get_docfield(me.doctype, d));
					} else if ((typeof d.fieldname!=='undefined') && (typeof d.type==='undefined')) {
						me.add_column(frappe.meta.get_docfield(me.doctype, d.fieldname));
						//me.total_colspans += parseInt(d.colspan);
					} else if ((typeof d.type!=='undefined')) {
						//me.columns.push(d);
						me.total_colspans += parseInt(d.colspan);
					}
				}
		...
		var empty_cols = flt(this.total_columns - this.total_colspans);
		while (empty_cols > 0) {
		...
		// Monkey Here - Removed Limiting
		//this.columns = this.columns.slice(0, 4);
	},
	add_column: function (df, cs) {
		// field width
		var colspan = 3;
	...
	get_header_html: function () {
		//Monkey Here _checkbox
		var main = this.columns.map(column =>
			frappe.render_template('list_item_main_head', {
				col: column,
				_checkbox: (typeof this.settings.selectable!== 'undefined') ? this.settings.selectable :(frappe.model.can_delete(this.doctype)
					&& !this.no_delete)
			})
		).join("");
	...
	prepare_data: function (data) {
		...
		//Monkey Here - _CheckBox
		data._checkbox = (typeof this.settings.selectable !== 'undefined') ? this.settings.selectable : (frappe.model.can_delete(this.doctype) && !this.no_delete)
		...
		//Monkey Here
		data._hide_activity = this.settings.hide_activity || 0;
		if (data._hide_activity) {
			$('.list-item__content.list-item__content--activity.hidden-xs.text-right.list-row-right').addClass('hide')
		}
	...
	insert_flex: function() {
		//Monkey
		$('.list-item.list-item--head').find('.list-item__content.hidden-xs.hidden-sm').addClass('head-name-flex');
	}
});
