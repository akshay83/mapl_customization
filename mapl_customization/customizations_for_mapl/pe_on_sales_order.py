...
def make_payment_entry_with_sales_order(doc, method):
	if doc.amended_from:
		doc.bank = None
		doc.amount_paid = None
		doc.customer_account = None
		doc.collection_account = None
		doc.mode_of_payment = None
		doc.reference_no = None
		doc.reference_date = None
		doc.is_payment_received = 0
		doc.db_update()
		frappe.msgprint("""Did not Create a Payment Entry as the Sales Order was Amended from a Cancelled Sales Order""")	
		return
	...
	payment_entry.paid_to = doc.collection_account
	payment_entry.paid_from = doc.customer_account
	payment_entry.paid_amount = doc.amount_paid
	payment_entry.received_amount = doc.amount_paid
	payment_entry.reference_no = doc.reference_no
	payment_entry.reference_date = doc.reference_date
	payment_entry.cheque_bank = doc.bank
	...
	ref.allocated_amount = doc.amount_paid
