...
def purchase_invoice_before_validate(doc, method):
	...
	for i in doc.items:
		...
		temp_expense_account = frappe.db.get_value("Item Default", { "parent": i.item_code, "company": doc.company }, "expense_account")
		if not temp_expense_account and cint(is_stock_item):
		...
		elif not temp_expense_account and not cint(is_stock_item) and not i.expense_account:
			frappe.throw("Please Set Expense Account")
		elif temp_expense_account and not i.expense_account:
			i.expense_account = temp_expense_account
